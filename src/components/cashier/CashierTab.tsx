...
export const CashierTab = () => {
  ...
  const handleCreatePayment = async (data: PaymentData) => {
    const paymentPayload = {
      ...data,
      amount: parseFloat(data.amount),
      proofFile: data.proofFile || undefined,
    };
  ...
  return (
    <div className="space-y-6">
                ...
                payments.map((payment) => (
                  <tr key={payment.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="text-xs text-gray-500">{payment.patient?.idNumber}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {payment.reference || '-'}
                    </td>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Record New Payment">
        <PaymentForm onSubmit={handleCreatePayment} onCancel={() => setIsModalOpen(false)} />
      </Modal>
    </div>
  );
};
