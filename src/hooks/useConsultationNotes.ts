...
export const useConsultationNotes = () => {
  ...
  const addConsultationNote = useCallback(
    async (data: AddConsultationData): Promise<HookResult> => {
      try {
  ...
  const updateConsultationNote = useCallback(
    async (id: string, data: Partial<AddConsultationData>): Promise<HookResult> => {
      try {
        ...
        const { error } = await supabase.from('consultation_notes').update(updateData).eq('id', id);
  ...
  const deleteConsultationNote = useCallback(
    async (id: string): Promise<HookResult> => {
      try {
  ...
  const getConsultationsByPatient = useCallback(
    (patientId: string) => {
